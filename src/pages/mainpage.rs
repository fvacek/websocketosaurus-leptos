...
pub fn MainPage() -> impl IntoView {
    ...
    view! {
        <Box style="display: flex; flex-direction: column; align-items: center; padding: 1em; min-height: 100%; min-width: 100%">
            ...
            <div>
                "url"
                <input type="text" size="50" value="wss://nirvana.elektroline.cz:37778?user=test&password=test" node_ref=input_url/>
            </div>
            <Stack orientation=StackOrientation::Horizontal spacing=Size::Em(0.6)>
                <span>"socket connected: " {move || is_socket_connected.get()}</span>
                <ButtonGroup>
                    <Button on_click=move|_| set_connect.set(true) disabled=Signal::derive(move|| is_connect.get())>
                        "Connect"
                    </Button>
                    <Button on_click=move|_| set_connect.set(false) disabled=Signal::derive(move|| !is_connect.get())>
                        "Disconnect"
                    </Button>
                </ButtonGroup>
            </Stack>
            {move || if is_connect.get() {
                let url = input_url.get().expect("element").value();
                view! {
                    <Wss url=url socket_connected=set_socket_connected />
                }
            } else {
